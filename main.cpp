...
int main(int argc, char *argv[])
{
    ...
    PauseMenu pauseMenu;
    bool isPaused = false; // Thêm biến isPaused

    // Nút pause kích thước 50x50, đặt ở góc dưới bên phải
    int pauseButtonWidth = 50;
    int pauseButtonHeight = 50;
    int pauseButtonX = SCREEN_WIDTH - pauseButtonWidth - 10;
    int pauseButtonY = SCREEN_HEIGHT - pauseButtonHeight - 10;
...
//        int x = stt-1;
    ...
    int num_die = 0;
    bool is_quit = false;
    int lifeBonusThreshold = 7; // Ngưỡng táo để nhận thêm mạng
    int maxLives = 3;
    int initialLives = 3;  // Số mạng ban đầu
    ...
    while(!is_quit)
    {
        ...
        while(SDL_PollEvent(&g_event) != 0)
        {
            ...
            if (pauseButton.handleEvent(&g_event, pauseButtonX, pauseButtonY, pauseButtonWidth, pauseButtonHeight))
            {
                isPaused = !isPaused; // Toggle pause state on button click
                if (isPaused)
                {
                    Mix_PauseMusic();
                }
        ...
        if (!isPaused) // Chỉ cập nhật và vẽ game nếu không tạm dừng
        {
        ...
        pauseButton.renderButton(g_screen, pauseButtonX, pauseButtonY);
        if (isPaused)
        {
            pauseMenu.renderPauseMenu(g_screen, SCREEN_WIDTH, SCREEN_HEIGHT);
        }
    ...
    g_hit_sound = NULL;
    player_.FreeSound();
    pauseButton.freeButtonTexture();
    pauseMenu.freeTextures();
    Mix_Quit();
    close();
    return 0;
}
